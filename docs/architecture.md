...
Third-party pi extensions are declared in `package.json` and referenced in `pi.extensions`. Waldemar currently bundles `pi-mcp-adapter` for general MCP compatibility surfaces and the `/usage` dashboard from `pi-extensions/usage-extension`. CodeGraph itself is exposed through a native `extensions/codegraph.ts` wrapper around `codegraph serve --mcp`. Sentry access is handled by external skills and the Sentry CLI, not MCP.
