...
export default function doctorExtension(pi: ExtensionAPI) {
  pi.registerCommand("waldemar-doctor", {
    ...
    handler: async (_args, ctx) => {
      ...
      await showDoctorReport(ctx, checks);
    },
  });
}

function runDoctorChecks(): DoctorCheck[] {
...
async function showDoctorReport(ctx: ExtensionContext, checks: DoctorCheck[]) {
  const pass = checks.filter((check) => check.status === "pass").length;
  const warn = checks.filter((check) => check.status === "warn").length;
  const fail = checks.filter((check) => check.status === "fail").length;

  if (ctx.mode !== "tui") {
    ctx.ui.notify(buildPlainReport(checks, pass, warn, fail), fail ? "error" : warn ? "warning" : "info");
    return;
  }

  await ctx.ui.custom<void>((_tui, theme, _keybindings, done) => {
    const borderColor = (text: string) => theme.fg("borderAccent", text);
    const container = new DoctorPanel(checks, { pass, warn, fail }, theme, borderColor, done);
    return container;
  });
}

function buildPlainReport(checks: DoctorCheck[], pass: number, warn: number, fail: number): string {
  return [
    "⚔️ Waldemar Doctor",
